...
import { calculateSiteMetrics } from '@/lib/data/mockSites'
...
export default function SitesPage() {
  ...
  // Filter sites by site group and customer context
  const customerFilteredSites = useMemo(() => {
    // First get sites for current site group
    const siteGroupSites = getSitesBySiteGroup(currentSiteGroup)
    // Then filter by customer
    return filterSitesByCustomer(siteGroupSites, activeCustomerId)
  ...
  const availableCustomers = useMemo(() => {
    // Get sites for current site group
    const siteGroupSites = getSitesBySiteGroup(currentSiteGroup)
    // Get unique customer IDs from site group sites
    const uniqueCustomerIds = Array.from(new Set(siteGroupSites.map(site => site.customerId)))
