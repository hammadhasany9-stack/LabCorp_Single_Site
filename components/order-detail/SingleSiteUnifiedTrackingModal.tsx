'use client'

import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog'
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select'
import { Copy, Check, Truck, Package, ChevronLeft, ChevronRight } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { useState, useEffect } from 'react'
import { CustomRequisition, TrackingStatus, TrackingStep } from '@/lib/types/order-detail'
import { TrackingStatusStepper } from './TrackingStatusStepper'

interface SingleSiteUnifiedTrackingModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  outboundTrackingIds: string[]
  outboundCarrier: string
  requisitions: CustomRequisition[]
  inboundCarrier: string
  initialTab?: 'outbound' | 'inbound'
  initialOutboundIndex?: number
  initialInboundIndex?: number
}

// Generate mock tracking data for outbound shipment
const generateOutboundTrackingData = (): { currentStatus: TrackingStatus; steps: TrackingStep[] } => {
  const currentStatus: TrackingStatus = 'delivered'
  const baseDate = new Date()
  baseDate.setDate(baseDate.getDate() - 3)
  
  const steps: TrackingStep[] = [
    {
      status: 'label_created',
      timestamp: new Date(baseDate.getTime())
    },
    {
      status: 'in_transit',
      timestamp: new Date(baseDate.getTime() + 12 * 60 * 60 * 1000)
    },
    {
      status: 'out_for_delivery',
      timestamp: new Date(baseDate.getTime() + 48 * 60 * 60 * 1000)
    },
    {
      status: 'delivered',
      timestamp: new Date(baseDate.getTime() + 60 * 60 * 60 * 1000)
    }
  ]
  
  return { currentStatus, steps }
}

export function SingleSiteUnifiedTrackingModal({
  open,
  onOpenChange,
  outboundTrackingIds,
  outboundCarrier,
  requisitions,
  inboundCarrier,
  initialTab = 'outbound',
  initialOutboundIndex = 0,
  initialInboundIndex = 0
}: SingleSiteUnifiedTrackingModalProps) {
  const [activeTab, setActiveTab] = useState<'outbound' | 'inbound'>(initialTab)
  const [outboundIndex, setOutboundIndex] = useState(initialOutboundIndex)
  const [inboundIndex, setInboundIndex] = useState(initialInboundIndex)
  const [copiedOutbound, setCopiedOutbound] = useState(false)
  const [copiedInbound, setCopiedInbound] = useState(false)
  
  // Update indices when modal opens with new initial values
  useEffect(() => {
    if (open) {
      setActiveTab(initialTab)
      setOutboundIndex(initialOutboundIndex)
      setInboundIndex(initialInboundIndex)
    }
  }, [open, initialTab, initialOutboundIndex, initialInboundIndex])

  const currentOutboundTrackingId = outboundTrackingIds[outboundIndex]
  const currentRequisition = requisitions[inboundIndex]
  const outboundTrackingData = generateOutboundTrackingData()

  // Outbound navigation
  const handleOutboundPrevious = () => {
    setOutboundIndex((prev) => (prev > 0 ? prev - 1 : outboundTrackingIds.length - 1))
    setCopiedOutbound(false)
  }

  const handleOutboundNext = () => {
    setOutboundIndex((prev) => (prev < outboundTrackingIds.length - 1 ? prev + 1 : 0))
    setCopiedOutbound(false)
  }

  const handleOutboundTrackingChange = (trackingId: string) => {
    const index = outboundTrackingIds.findIndex(id => id === trackingId)
    if (index !== -1) {
      setOutboundIndex(index)
      setCopiedOutbound(false)
    }
  }

  // Inbound navigation
  const handleInboundPrevious = () => {
    setInboundIndex((prev) => (prev > 0 ? prev - 1 : requisitions.length - 1))
    setCopiedInbound(false)
  }

  const handleInboundNext = () => {
    setInboundIndex((prev) => (prev < requisitions.length - 1 ? prev + 1 : 0))
    setCopiedInbound(false)
  }

  const handleControlIdChange = (controlId: string) => {
    const index = requisitions.findIndex(req => req.controlId === controlId)
    if (index !== -1) {
      setInboundIndex(index)
      setCopiedInbound(false)
    }
  }

  // Copy handlers
  const handleCopyOutbound = async () => {
    if (!currentOutboundTrackingId) return
    await navigator.clipboard.writeText(currentOutboundTrackingId)
    setCopiedOutbound(true)
    setTimeout(() => setCopiedOutbound(false), 2000)
  }

  const handleCopyInbound = async () => {
    if (!currentRequisition) return
    await navigator.clipboard.writeText(currentRequisition.inboundTrackingId)
    setCopiedInbound(true)
    setTimeout(() => setCopiedInbound(false), 2000)
  }

  // Generate carrier-specific tracking URL
  const getTrackingUrl = (trackingId: string, carrier: string) => {
    const carrierLower = carrier.toLowerCase()
    if (carrierLower.includes('fedex')) {
      return `https://www.fedex.com/fedextrack/?trknbr=${trackingId}`
    } else if (carrierLower.includes('ups')) {
      return `https://www.ups.com/track?tracknum=${trackingId}`
    } else if (carrierLower.includes('usps')) {
      return `https://tools.usps.com/go/TrackConfirmAction?tLabels=${trackingId}`
    }
    return `#`
  }

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="sticky top-0 z-10 w-full py-4 mt-4 flex items-center backdrop-blur-sm dark:backdrop-blur-sm">
          <DialogTitle className="text-xl font-bold text-gray-900 dark:text-gray-50 text-center">
            Package Tracking
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-6 py-2">
          {/* Tabs for Outbound/Inbound */}
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'outbound' | 'inbound')}>
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="outbound">Outbound Shipment</TabsTrigger>
              <TabsTrigger value="inbound">Inbound Shipment</TabsTrigger>
            </TabsList>

            {/* Outbound Tab Content */}
            <TabsContent value="outbound" className="space-y-6 mt-4">
              {/* Navigation Header */}
              <div className="flex items-center justify-between gap-4">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={handleOutboundPrevious}
                  className="h-8 w-8 rounded-xl bg-gray-200 dark:bg-zinc-800 hover:bg-gray-100 dark:hover:bg-zinc-800 flex-shrink-0"
                  disabled={outboundTrackingIds.length <= 1}
                >
                  <ChevronLeft className="h-5 w-5" />
                </Button>

                <div className="flex-1 text-center">
                  {outboundTrackingIds.length > 1 && (
                    <p className="text-xs text-gray-500 dark:text-gray-500">
                      Showing {outboundIndex + 1} of {outboundTrackingIds.length} tracking numbers
                    </p>
                  )}
                </div>

                <Button
                  variant="ghost"
                  size="icon"
                  onClick={handleOutboundNext}
                  className="h-8 w-8 bg-gray-200 dark:bg-zinc-800 rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 flex-shrink-0"
                  disabled={outboundTrackingIds.length <= 1}
                >
                  <ChevronRight className="h-5 w-5" />
                </Button>
              </div>

              {/* Tracking ID Selector - Only show if multiple tracking IDs */}
              {outboundTrackingIds.length > 1 && (
                <div className="flex justify-center pb-4 border-b border-gray-200 dark:border-zinc-800">
                  <Select value={currentOutboundTrackingId} onValueChange={handleOutboundTrackingChange}>
                    <SelectTrigger className="w-[400px] h-10">
                      <div className="flex items-center w-full">
                        <span className="font-mono font-semibold text-sm">{currentOutboundTrackingId}</span>
                      </div>
                    </SelectTrigger>
                    <SelectContent>
                      {outboundTrackingIds.map((trackingId, index) => (
                        <SelectItem key={trackingId} value={trackingId}>
                          <div className="flex items-center justify-between gap-4 w-full">
                            <span className="font-mono font-semibold">{trackingId}</span>
                            <span className="text-xs text-gray-500 dark:text-gray-400">
                              #{index + 1}
                            </span>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
              )}

              {/* Carrier Info */}
              <div className="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-950 rounded-xl">
                <div className="flex-shrink-0 w-10 h-10 bg-green-100 dark:bg-green-950 rounded-full flex items-center justify-center">
                  <Truck className="h-5 w-5 text-green-600 dark:text-green-400" />
                </div>
                <div className="flex-1">
                  <label className="text-xs text-green-700 dark:text-green-300 font-medium uppercase">
                    Carrier Type
                  </label>
                  <p className="text-sm font-semibold text-green-900 dark:text-green-50 mt-0.5">
                    {outboundCarrier}
                  </p>
                </div>
              </div>

              {/* Outbound Shipment Details */}
              <div className="space-y-3">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-50 uppercase tracking-wide">
                  Shipment to Client Location
                </h3>
                
                <div className="flex items-center gap-2 p-3 bg-white dark:bg-card border-2 border-gray-200 dark:border-zinc-800 rounded-lg">
                  <div className="flex-1 min-w-0">
                    <label className="text-xs text-gray-600 dark:text-gray-400 font-medium">
                      Tracking ID
                    </label>
                    <p className="text-sm font-mono font-semibold text-gray-900 dark:text-gray-50 mt-0.5 break-all">
                      {currentOutboundTrackingId}
                    </p>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={handleCopyOutbound}
                    className="h-9 w-9 p-0 flex-shrink-0"
                  >
                    {copiedOutbound ? (
                      <Check className="h-4 w-4 text-green-600" />
                    ) : (
                      <Copy className="h-4 w-4" />
                    )}
                  </Button>
                </div>

                {/* Tracking Link */}
                <div className="pt-1">
                  <a
                    href={getTrackingUrl(currentOutboundTrackingId, outboundCarrier)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center text-sm text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 hover:underline"
                  >
                    Track on {outboundCarrier} Website →
                  </a>
                </div>
              </div>

              {/* Divider */}
              <div className="border-t border-gray-200 dark:border-zinc-800" />

              {/* Tracking Status Stepper */}
              <div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-50 uppercase tracking-wide mb-4">
                  Shipment Status
                </h3>
                <TrackingStatusStepper
                  steps={outboundTrackingData.steps}
                  currentStatus={outboundTrackingData.currentStatus}
                />
              </div>

              {/* Info Box */}
              <div className="p-3 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-900">
                <p className="text-xs text-green-800 dark:text-green-200">
                  <strong>Note:</strong> Tracking information is provided by {outboundCarrier}. 
                  This shipment is being sent from our facility to the client location.
                </p>
              </div>
            </TabsContent>

            {/* Inbound Tab Content */}
            <TabsContent value="inbound" className="space-y-6 mt-4">
              {/* Navigation Header */}
              <div className="flex items-center justify-between gap-4">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={handleInboundPrevious}
                  className="h-8 w-8 rounded-xl bg-gray-200 dark:bg-zinc-800 hover:bg-gray-100 dark:hover:bg-zinc-800 flex-shrink-0"
                  disabled={requisitions.length <= 1}
                >
                  <ChevronLeft className="h-5 w-5" />
                </Button>

                <div className="flex-1 text-center">
                  {requisitions.length > 1 && (
                    <p className="text-xs text-gray-500 dark:text-gray-500">
                      Showing {inboundIndex + 1} of {requisitions.length} control IDs
                    </p>
                  )}
                </div>

                <Button
                  variant="ghost"
                  size="icon"
                  onClick={handleInboundNext}
                  className="h-8 w-8 bg-gray-200 dark:bg-zinc-800 rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 flex-shrink-0"
                  disabled={requisitions.length <= 1}
                >
                  <ChevronRight className="h-5 w-5" />
                </Button>
              </div>

              {/* Control ID Selector - Only show if multiple requisitions */}
              {requisitions.length > 1 && currentRequisition && (
                <div className="flex justify-center pb-4 border-b border-gray-200 dark:border-zinc-800">
                  <Select value={currentRequisition.controlId} onValueChange={handleControlIdChange}>
                    <SelectTrigger className="w-[400px] h-10">
                      <div className="flex items-center w-full">
                        <span className="font-mono font-semibold text-sm">{currentRequisition.controlId}</span>
                      </div>
                    </SelectTrigger>
                    <SelectContent>
                      {requisitions.map((req, index) => (
                        <SelectItem key={req.controlId} value={req.controlId}>
                          <div className="flex items-center justify-between gap-4 w-full">
                            <span className="font-mono font-semibold">{req.controlId}</span>
                            <span className="text-xs text-gray-500 dark:text-gray-400">
                              #{index + 1}
                            </span>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
              )}

              {currentRequisition && (
                <>
                  {/* Control ID and Carrier Info */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Control ID */}
                    <div className="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-950 rounded-xl">
                      <div className="flex-shrink-0 w-10 h-10 bg-blue-100 dark:bg-blue-950 rounded-full flex items-center justify-center">
                        <Package className="h-5 w-5 text-blue-600 dark:text-blue-400" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <label className="text-xs text-blue-700 dark:text-blue-300 font-medium uppercase">
                          Control ID
                        </label>
                        <p className="text-sm font-mono font-semibold text-blue-900 dark:text-blue-50 mt-0.5 truncate">
                          {currentRequisition.controlId}
                        </p>
                      </div>
                    </div>

                    {/* Carrier Type */}
                    <div className="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-950 rounded-xl">
                      <div className="flex-shrink-0 w-10 h-10 bg-blue-100 dark:bg-blue-950 rounded-full flex items-center justify-center">
                        <Truck className="h-5 w-5 text-blue-600 dark:text-blue-400" />
                      </div>
                      <div className="flex-1">
                        <label className="text-xs text-blue-700 dark:text-blue-300 font-medium uppercase">
                          Carrier Type
                        </label>
                        <p className="text-sm font-semibold text-blue-900 dark:text-blue-50 mt-0.5">
                          {currentRequisition.carrierType || inboundCarrier}
                        </p>
                      </div>
                    </div>
                  </div>

                  {/* Inbound Shipment Details */}
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-50 uppercase tracking-wide">
                      Shipment to Lab
                    </h3>
                    
                    <div className="flex items-center gap-2 p-3 bg-white dark:bg-card border-2 border-gray-200 dark:border-zinc-800 rounded-lg">
                      <div className="flex-1 min-w-0">
                        <label className="text-xs text-gray-600 dark:text-gray-400 font-medium">
                          Tracking ID
                        </label>
                        <p className="text-sm font-mono font-semibold text-gray-900 dark:text-gray-50 mt-0.5 break-all">
                          {currentRequisition.inboundTrackingId}
                        </p>
                      </div>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={handleCopyInbound}
                        className="h-9 w-9 p-0 flex-shrink-0"
                      >
                        {copiedInbound ? (
                          <Check className="h-4 w-4 text-green-600" />
                        ) : (
                          <Copy className="h-4 w-4" />
                        )}
                      </Button>
                    </div>

                    {/* Tracking Link */}
                    <div className="pt-1">
                      <a
                        href={getTrackingUrl(currentRequisition.inboundTrackingId, currentRequisition.carrierType || inboundCarrier)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:underline"
                      >
                        Track on {currentRequisition.carrierType || inboundCarrier} Website →
                      </a>
                    </div>
                  </div>

                  {/* Divider */}
                  <div className="border-t border-gray-200 dark:border-zinc-800" />

                  {/* Tracking Status Stepper */}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-50 uppercase tracking-wide mb-4">
                      Shipment Status
                    </h3>
                    <TrackingStatusStepper
                      steps={currentRequisition.trackingSteps || []}
                      currentStatus={currentRequisition.currentStatus}
                    />
                  </div>

                  {/* Info Box */}
                  <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-900">
                    <p className="text-xs text-blue-800 dark:text-blue-200">
                      <strong>Note:</strong> Tracking information is provided by {currentRequisition.carrierType || inboundCarrier}. 
                      Delivery times may vary based on location and service type.
                    </p>
                  </div>
                </>
              )}
            </TabsContent>
          </Tabs>
        </div>
      </DialogContent>
    </Dialog>
  )
}
