...
export function Navbar({ title = "Dashboard" }: NavbarProps) {
  ...
  const userDisplay = session?.user?.email || session?.user?.name || "user@labcorp.com"
        ...
        <div className="flex h-16  items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/" className="cursor-pointer transition-opacity hover:opacity-80">
              <Image
                src={mounted && currentTheme === 'dark' ? logoWhite : logo}
                alt="Labcorp Logo"
                width={130}
                height={200}
                className="h-35"
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-60">
                <DropdownMenuLabel className="text-md">{userDisplay}</DropdownMenuLabel>
                <DropdownMenuSeparator />
                <Link href="/profile">
